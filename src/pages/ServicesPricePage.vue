...
const read = async () => {
  ...
  rows.value = response.data.showServiceCollection;
};
const deleteById = async (id: number) => {
  await graphqlRequest(`mutation deleteServiceCollection{
      deleteServiceCollection(id:${id})
    }`);
  await read();
};
...
const create = async (form: any) => {
  ...
  let q = `mutation
      createServiceCollection($input:ServiceCollectionInput!){
      createServiceCollection(
        input: {
        branchId: ${newData.branchId},
        serviceId: ${newData.serviceId},
        cost: ${
          newData.cost.indexOf('.') > -1 ? newData.cost : newData.cost + '.0'
        }
        })
    }`;
  console.log(q);
