...
const updateById = async (id: number, form: any) => {
  ...
  const res =
    await graphqlRequest(`mutation updateBranch($id: Int!, $branchInput: BranchDataInput!){
      updateBranch(id:${id}, branchInput:{name:" ${updatedData.name}",country: "${updatedData.country}",city: "${updatedData.city}",address: "${updatedData.address}",phoneNumber: "${updatedData.phoneNumber}",orgId: ${updatedData.orgId}})
    }`);
  console.log(res);
...
const create = async (form: any) => {
  ...
  let q = `mutation
      createBranch($branchInput: BranchDataInput!){
      createBranch(
        branchInput: {
        name:" ${newData.name}",
